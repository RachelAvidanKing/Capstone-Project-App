...
class FirebaseConnector:
        ...
        try:
            ...
            for participant_doc in participants_ref.stream():
                ...
                for trial_doc in trials_ref.stream():
                    ...
                    data['participantId'] = participant_id  # Ensure participantId is included
                    
                    # movementPath should already be an array in Firestore
                    # No need to parse if it's already a list
    ...
    def fetch_movement_data(self) -> pd.DataFrame:
        """Fetch movement tracking data (if you have this collection)"""
        print("Fetching movement data...")
        
        try:
            movement_ref = self.db.collection('movement_data')
            movement_data = []
            
            for doc in movement_ref.stream():
                data = doc.to_dict()
                data['dataId'] = doc.id
                movement_data.append(data)
            
            df = pd.DataFrame(movement_data)
            print(f"  → Loaded {len(df)} movement data points")
            return df
        except Exception as e:
            print(f"  → No movement_data collection found or error: {e}")
            return pd.DataFrame()
    ...
    def fetch_all_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        return participants_df, trials_df, movement_df
    ...
    def get_participant_trials(self, participant_id: str) -> pd.DataFrame:
        ...
        trials_ref = self.db.collection('target_trials').where('participantId', '==', participant_id)
    ...
    def get_trial_summary(self) -> Dict:
        ...
        participants_df, trials_df, _ = self.fetch_all_data()
...
def load_data(credentials_path: str = 'serviceAccountKey.json') -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
